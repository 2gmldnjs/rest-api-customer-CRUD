...
const getTotal = async () => {
  ...
  try {
    const query = `SELECT COUNT(*) AS cnt FROM ${TABLE.CUSTOMERS}`;
...
const getSelectOne = async (id) => {
  try {
    const query = `SELECT COUNT(*) AS cnt FROM ${TABLE.CUSTOMERS} WHERE id=?`;
    const values = [id];
...
const getList = async (req) => {
  ...
  try {
    ...
    const query = `SELECT * FROM ${TABLE.CUSTOMERS} ${where} order by id desc limit 0, ${len}`;
...
const customerController = {
  ...
  update: async (req) => {
    ...
    const { email , name } = req.body;
    if (isEmpty(id) || isEmpty(email) || isEmpty(name)) {
      return resData(STATUS.E100.result, STATUS.E100.resultDesc, moment().format('LT'));
    }

    try {
      const query = `UPDATE ${TABLE.CUSTOMERS} SET email =?, name=? WHERE id= ?`;
    ...
    delete: async (req) => {
      ...
      try {
        ...
        const query = `DELETE FROM ${TABLE.CUSTOMERS} WHERE id = ?;`;
        const values = [id];
        const [rows] = await db.execute(query, values);
        if (rows.affectedRows == 1) {
          return resData(
  ...
  //reset //테이블 내용 삭제
  reset: async (req) => {
    const query = `TRUNCATE TABLE ${TABLE.CUSTOMERS};`; 
    const [rows] = await db.execute(query);
    return rows
  },
    //turncate후 dummy 더미 데이터 추가 len인자 길이만큼
    dummy: async (req) => {
      const { email, name, len } = req.body; // url /로 들어오는 파라미터
      console.log(email,name, len)
      if (isEmpty(email) || isEmpty(len) || isEmpty(name)) {
        return resData(STATUS.E100.result, STATUS.E100.resultDesc, moment().format('LT'));
      }
      console.log(1)
      try {
        const query = `TRUNCATE TABLE ${TABLE.CUSTOMERS};`; 
        const values = [email,name,len];
        const [rows] = await db.execute(query, values);
